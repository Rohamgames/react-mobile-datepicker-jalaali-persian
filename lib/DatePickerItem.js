...
class DatePickerItem extends Component<void, Props, State> {
    ...
    componentDidMount() {
        ...
        viewport.addEventListener('touchcancel', this.handleContentTouch, false);
        viewport.addEventListener('mousedown', this.handleContentMouseDown, false);
    }

    componentWillReceiveProps(nextProps) {
        // if (nextProps.value.jy === this.props.value.jy && nextProps.value.jm === this.props.value.jm && nextProps.value.jd === this.props.value.jd) {
        //     return;
        // }
    ...
    _moveToNext(direction) {
        ...
        if (direction === -1 /*&& date.jy < min.jy && date.jm < min.jm && date.jd < min.jd*/ && this.moveDateCount) {
            this._updateDates(1);
        } else if (direction === 1 /*&& date.jy > max.jy && date.jm > max.jm && date.jd > max.jd*/ && this.moveDateCount) {
            this._updateDates(-1);
        }
    ...
    handleMove(event) {
        ...
        const date = this.state.dates[MIDDLE_INDEX];
        const { max, min } = this.props;
        // if (date.jy < min.jy || date.jm < min.jm || date.jd < min.jd ||
        //     date.jy > max.jy || date.jm > max.jm || date.jd > max.jd) {
        //     return;
        // }
    ...
    renderDatepickerItem(date, index) {
        ...
        return (
            <li
                // key={index+(date.jy+date.jm+date.jd)}
                className={className}>
                {formatDate}
            </li>
        );
                ...
                <div
                    ...
                    <div className="datepicker-wheel">
                        <ul
                            ...
                            style={scrollStyle}>
                            {this.state.dates.map((item,index)=>(this.renderDatepickerItem(item,index)))}
                        </ul>
                    </div>
                </div>
            </div>
